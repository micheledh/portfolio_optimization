...
var resizePizzas = function(size) {
  ...
  // no need of determineDx function anymore
  ...
  function changePizzaSizes(size) {
  	...
  	// create a variable to avoid repetitions, and avoid creating the DOM for each iteration
...
function updatePositions() {
  ...
  // var outside of the loop, so DOM isn't needed for each iteration
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var numPizzaCols = Math.ceil(width / 200);
  var height = window.innerHeight;
  var s = 256;
  var numPizzaRow = Math.ceil(height / s);
  //there are not 200 pizzas on the screen, 50 is enough
  for (var i = 0; i < numPizzaRow * numPizzaCols; i++) {
    var elem = document.createElement('img');
    elem.className = 'mover';
    elem.src = "images/pizza.png";
    elem.style.height = "100px";
    elem.style.width = "73.333px";
    elem.style.left = (i % numPizzaCols) * s + 'px';
    elem.style.top = (Math.floor(i / numPizzaCols) * s) + 'px';
